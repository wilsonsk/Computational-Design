...
- **One-to-One Mapping**: Each input $\LARGE x$ in the domain maps to one and only one output $\LARGE y=f(x)$. This rule ensures that a function provides a clear, unambiguous output for every input.
